...
class FileSystem {
    ...
    // Print file system
    print() {
        console.log(util.inspect(this.fileSystem, {showHidden: false, depth: null, colors: true}));
    }
}
...
fs.readFile("./input.txt", "utf8", (err, data) => {
    ...
    console.log(smallDir.reduce((sum, size) => sum + size, 0));
});
...
const getDirectoriesSize = (node, sizesHolder = []) => {
    ...
    const size = node.children.reduce((sum, child) => sum + getDirectoriesSize(child, sizesHolder), 0);
